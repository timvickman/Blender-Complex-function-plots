...
bl_info = {
    "name": "Riemann Surface (Imaginary)",
     "description": "Plots a complex-valued function. Z coordinate represents the imaginary component of the output.",
    "author": "Lemmaxiom",
    "version": (1, 0),
    "blender": (2, 91, 0),
...
class MESH_OT_imag_rsurf(bpy.types.Operator):
    """Create a Riemann Surface (Imaginary Representation)"""   # Operator tool-tips description
    bl_idname = "mesh.riemann_imag_add"     # Operator ID name (for typing in console)
    bl_label = "Imaginary Riemann Surface"          # Operator name (for searching using F3 shortcut)
...
def add_button(self,context):
    self.layout.operator(
        MESH_OT_imag_rsurf.bl_idname,
        text = "Riemann Surface (Imaginary)",
        icon = 'SURFACE_NSURFACE')

def register():
    bpy.utils.register_class(MESH_OT_imag_rsurf)
    bpy.types.VIEW3D_MT_mesh_add.append(add_button)

def unregister():
    bpy.utils.unregister_class(MESH_OT_imag_rsurf)
